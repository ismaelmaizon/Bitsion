...
export default function UpdatePersona () {
    ...
    const { persona, updatePersona,alert} = useContext(MiContexto)
        ...
        <Box sx={{ width: '60%', margin: 'auto', marginTop: '120px', padding: '15px', boxShadow: '2px 2px 10px 2px' }}  >
            ...
            <Grid container direction='row' marginBottom="70px" marginTop="10px">
                <Grid item xs={10} container direction='row' spacing={3} sx={{ margin:'auto' }} >                        
                        <Grid item xs={6}>
                        <TextField fullWidth label={persona.nombre_completo} name='nombre_completo' type="text" onChange={dataFrom}></TextField>
                        </Grid>
                        <Grid item xs={6}>
                        <TextField fullWidth label={persona.identificacion} name='identificacion' type="text" onChange={dataFrom}></TextField>
                        </Grid>
                        <Grid item xs={6}>
                            <TextField fullWidth label={persona.edad} name='edad' type="text" onChange={dataFrom}></TextField>
                        </Grid>
                </Grid>
                <Grid item xs={10} container direction="row" spacing={3} sx={{ margin:'auto' }}>
                        <Grid item xs={6}>
            ...
            <Button type="submit" variant="contained" size="small" sx={{ width:'200px', margin: 'auto' }} onClick={ async ()=>{
                console.log(data);
                let respon = await updatePersona(data)
